#' @name snowball_sampling_function.R
#' @date 19.01.2026
#' @author Marc Kevin Schneider
#' @contact Schnei7q@students.uni-marburg.de
#' 
#' @description
#' Functions for snowball sampling the presence points and absence points
...
# ================================================================
# 2. Snowball Sampling function
# ================================================================

snowball_sampling <- function(species_name, fit, sample_p, iter){
  '
  Purpose: Samples presence-absence points using a snowball sampling approach with
           a 250km radius for the initial points
  ...
  Returns:
  ---------------------------
  A presence-absence dataset with a snowball sampling approach bias
  ...
  init_pts <- sampleOccurrences(presence, n=2, 
                                type="presence-absence", replacement=FALSE,
                                sample.prevalence=0.5,
                                plot=FALSE)
  
  # extract coordinates as a data frame
  init_df <- as.data.frame(init_pts$sample.points)
  
  # convert to sf point object
  init_sf <- sf::st_as_sf(init_df, coords = c("x", "y"), crs = terra::crs(landscape))
  
  # buffer points by 250 km
  ...
  dir_pres <- paste0(envrmt$path_pre_abs_points, "/Snowball/", species_name, "/", sample_p)
